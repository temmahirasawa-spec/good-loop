...
const breaches = [
  ...
  // レビューで挙がった再現を、報告されたとおりの文字列で全部固定する
  ["意味の反転を通さない（短かった→短くなかった）", "待ち時間短かった", "待ち時間は短くなかった。", false],
  ["意味の反転を通さない（ぜんぶよかった）", "ぜんぶよかった", "ぜんぶよくなかった。", false],
  ["意味の反転を通さない（単文）", "接客よかった", "接客よくなかった。", false],
  ["。と！を混ぜて3文にした出力を通さない", "料理早い 接客よかった 店きれい", "料理が早い！接客もよかった。店もきれい！", false],
  ["？を足して断定を疑問に変える出力を通さない", "接客よかった", "接客よかった？", false],
  ["ます を増やす出力を通さない", "また来ます 接客よかった", "また来ます、接客もよかったます", false],
  ["です を複数足す出力を通さない", "はじめてです 接客よかった 料理も早かった", "はじめてです、接客がよかったです、料理も早かったです", false],
  // AIの出力をもう一度入力にして積み上げる経路（UI側でも採用は1回までにしてある）
  ["多段で語を足す出力を通さない", "料理は早い、席もひろい。", "料理はとても早い、席もひろい。", false],
  // 本人が最初から否定を書いている文は巻き添えにしない
  ["本人が書いた否定はそのまま通る①", "混んでなかった 待ち時間短い", "混んでなかった、待ち時間は短い。", true],
  ["本人が書いた否定はそのまま通る②", "おいしくなかった", "おいしくなかった。", true],
  ...
  ["実測の出力はそのまま通る⑥", "パンケーキ ふわふわ 生クリームも多め", "パンケーキはふわふわで、生クリームも多め。", true],
  ["実測の出力はそのまま通る⑦", "店員さん 忙しそうだったけど笑顔だった", "店員さんは忙しそうだったけど、笑顔だった。", true],
  ["実測の出力はそのまま通る⑧", "今日はじめて来た 駅から近くてよかった", "今日はじめて来たが、駅から近くてよかった。", true],
];
let breachFailed = 0;
