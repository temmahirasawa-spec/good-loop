...
if (existsSync(tokenPath)) {
  for (const line of readFileSync(tokenPath, "utf8").split("\n")) {
...
if (outside.length) {
  ...
  console.error("  トークンのどれとも一致しません。業態テーマを切り替えたとき、ここだけ取り残されます。");
  console.error(`  → ${TOKEN_FILE} に追加するか、既存トークンに寄せてください。`);
