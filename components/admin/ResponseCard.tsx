    ...
    >
      <div className="flex w-full items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <p className="whitespace-nowrap text-[15px] font-bold" style={{ color: ratingColor }}>
            {"★".repeat(response.rating)}
            {"☆".repeat(5 - response.rating)}
          </p>
          {showStoreName && (
            <p className="whitespace-nowrap text-sm font-bold" style={{ color: "var(--product-color-text-primary)" }}>
              {response.storeName}
            </p>
          )}
        ...
        <div
          className="flex items-start rounded-full px-3 py-1"
          style={{
      ...
      <div className="flex items-start gap-2">
        {response.tags.map((tag) => (
          <div key={tag} className="flex items-start rounded-lg px-3 py-1" style={{ backgroundColor: "var(--product-color-bg-primary)" }}>
