...
画面に見える変更を加えた場合は、`npm run check` に加えて、スクリーンショットを撮り、
それを提示するまでを完了とする。撮るのは **PC幅 1400px** と **スマホ幅 390px** の2枚。

**撮影は `npm run screenshot -- <path>` を使う（`scripts/screenshot.mjs`）。**
Playwright のヘッドレスChromiumで撮るため、天真の実ブラウザには一切触れない。
`claude-in-chrome` で天真の実ブラウザを操作するのは、天真から明示的に「実際のブラウザで
動きを見せて」等と頼まれたときだけにする（2026-08-05、実ブラウザを繰り返し操作して
迷惑をかけた反省）。

dev サーバーが起動していない場合は、**起動を求めて止まる**こと（勝手にバックグラウンドで
起動しない）。`npm install` をdevサーバー起動中に行うと `.next` のキャッシュが壊れることが
あるため、パッケージを追加・削除するときはdevサーバーを止めるよう伝えること。
