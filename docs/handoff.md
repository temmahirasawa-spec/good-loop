...
> 追加して497件が乗り、現在は500件。台帳は下の「Figma の負債台帳」。**
...
| ベースライン | 空（0件）で新規作成。GOOD ORDER の51件は持ち込んでいない（その後の増減は後述の台帳を参照。2026-08-05 時点で500件） |
...
**現在の台帳は 500件 / 312種類。** `npm run design:figma` は緑だが、
それは「未返済の負債が500件ある」ことを意味する。

| 種類 | 件数 | 中身 |
|---|---|---|
| 余白がスケール外 | **497** | **大半が探索用サンプル**（下の表） |
...
| `Web Design Master / 01 LP / GOOD LOOP` | **0** | 同上 |
| `Components / 05 Web Components` | **0** | 同上 |
| `01 Rating UI Exploration / 評価UI 5案` | 109 | **探索用**（5案の比較検討） |
| `02-B`〜`02-I`（業態サンプル 8セクション） | 各40 ＝ **320** | **サンプル**（業態ごとの色違い） |
| `Components / 04 Theme Palettes` | 54 | **サンプル**（色見本） |
| `Components / 03 Components / GOOD LOOP` | 14 | コンポーネント置き場 |

**探索用・サンプル（`01 Rating` ＋ `02-B`〜`02-I` ＋ `04 Theme Palettes`）だけで 483件。
497件の 97% を占める。** ここは案を並べて比べるための場所で、本番の画面ではない。
