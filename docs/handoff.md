...
**Figma側は未対応。** 天真は「Figmaにも実装にも作ってほしい」と発言している。現在の01/02/04の
フレームは（ボタンが常に活性の状態で）作られているため、初期状態（非活性）のCTAボタンに
差し替える必要がある。`Loop/Button` コンポーネント自体に `Primary/Disabled` バリアントは既に
あるので、新規デザインではなく既存インスタンスのバリアント差し替えで済む見込み。次のセッションで
天真に「コード側は直したので、Figma側もこの状態に合わせて直してよいか」を確認してから着手する。
