...
- ファイル上の件数は **27**、見逃している違反は **54**。
  ベースラインは「セクション名＋メッセージ」の**重複を除いて**記録するため
  （`CTA Block` が同じセクションに2つあっても1行になる）。数が合わないのは正常
- ⚠ **限界**: 同じセクションに同じ名前のノードが1つ増えても、キーが同じなので検出されない。
  「新しい種類の違反」は落ちるが、「同じ違反が増えたこと」は落ちない
