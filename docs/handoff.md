...
1. **天真の判断** — 資産の質 **69件**をベースラインに登録するか。
   48件の想定に対して21件多い。**構造違反の混入ではなく**、天真の Figma 修正によって
   検査が届く範囲が広がった結果（内訳は下記）
...
### 48件 → 69件 の内訳（構造違反は1件も混ざっていない）

`--update-baseline` は `soft`（資産の質）だけを書き出す。構造違反は原理的に混入しない。

| 増えた分 | 件数 | なぜ増えたか |
|---|---|---|
| `06 Dashboard / SP` のタップ領域44px未満 | **+15** | セクション名が `06-SP / Mobile 390` → `SP` に改名された。検品は名前がちょうど `SP` のときだけ SP として扱うので、改名前はタップ領域の検査が走っていなかった |
| `Web Design Master / 01 LP / GOOD LOOP / PC` の生フレーム | **+6** | ページがセクションで包まれた。以前は「セクション未使用」でページごとスキップされていた |

48 ＋ 15 ＋ 6 ＝ 69。**どちらも「新しく壊れた」のではなく「今まで見えていなかったものが見えた」。**
