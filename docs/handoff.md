...
### ⚠ 未解決 — `08 プレゼン` セクションの扱いが決まっていない

この作業の**最中に** `08 プレゼン / 洋輔さんMTG（2026-08-06）` が Figma に追加された
（21枚のスライド）。ベースラインを 252件で作り直した**あと**に現れたため、
**台帳には入っていない。いま `npm run design:figma` は赤である。**

```
✗ 構造・パディング・セクション色（4件）
  App Design Master / 08 プレゼン / 洋輔さんMTG（2026-08-06）
    ・大枠のセクションの色が #EDEDED です（#7E7E7E にしてください）
    ・PC セクションがありません / SP セクションがありません
    ・PC / SP の外に直接置かれた要素があります: 「Slide 01 — タイトル」…（21枚）
✗ 新しい種類の違反（908件 / 398種類）
```

**これはプレゼン資料であって画面ではない。** 画面制作ページ（`App Design Master`）の
規約（PC/SP の対、セクション色、100pxパディング）をそのまま当てるのは筋が違う。

**扱いを天真が決める必要がある。** 選択肢は3つ。

1. **Figma で `99 プレゼン / 洋輔さんMTG（2026-08-06）` にリネームする（推奨）**
   検品には既に「`99 〜` で始まるセクションは素材置き場・メモ置き場とみなす」という
   仕組みがある。構造4件はこれで消える。**コードを足さずに済む**
2. プレゼンを別ページに切り出し、`SKIP_PAGES` に足す
3. 検品スクリプトに「プレゼン用セクション」の除外を新設する

いずれの場合も、余白違反908件は残るので、構造が0件になってから台帳に載せ直すこと。
...
1. **天真の判断** — `08 プレゼン` セクションの扱い（上記。`99 〜` へのリネームを推奨）。
   決まって構造が0件になったら、余白違反908件を台帳に載せ直す
