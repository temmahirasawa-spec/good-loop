...
### 余白のスケール検査を追加した（2026-08-05）— **台帳はまだ作り直していない**
...
### 検出結果（2026-08-05 実測・497件）

**掃除済みのセクションは0件。** 天真が Cowork 側でバインドした範囲と一致した。

| セクション | 余白違反 | 備考 |
|---|---|---|
| `02 基本形 / 画面遷移図` | **0** | 掃除済み（89箇所をバインド） |
| `07 管理画面 / Dashboard` | **0** | 掃除済み（329箇所をバインド） |
| `06 Dashboard / 全店一覧 3案` | **0** | 未処理と聞いていたが、実測では既にきれいだった |
| `Web Design Master / 01 LP` | **0** | 同上 |
| `05 Web Components` | **0** | 同上 |
| `01 Rating UI Exploration` | 109 | 探索用。直さずベースラインに登録する |
| `02-B`〜`02-I`（業態サンプル） | 各40（計320） | 同上 |
| `04 Theme Palettes` | 54 | 同上 |
| `03 Components / GOOD LOOP` | 14 | 同上 |

対象範囲が正しいことは裏を取ってある。`06 Dashboard` は433個、`Web Design Master` は115個の
オートレイアウトを**検査した上で**0件であり、見落としではない。

**まだ `--update-baseline` を実行していない。** 構造違反が2件残っているため（下記）。
構造が残ったまま台帳を作ると、構造の穴が台帳に載って見えなくなる。

### ⚠ 未解決 — `02 基本形` の構造違反2件（Figma 側の修正が必要）

```
App Design Master / 02 基本形 / 画面遷移図（390×844）
  ・上パディングが 120px です（100px）
  ・右パディングが 590px です（100px）
```

セクション枠は `2060x2108`。左と下は 100px で規約どおり。**ずれているのは上と右だけ。**
改名と同時に中身が画面遷移図へ作り替わったときに生じたと見られる。

あわせて、同セクションに新しいソフトエラーが1件出ている。
`「Loop / Button」の高さが 36px です（SPのタップ領域は44px以上）`。
**これは本物の指摘**（SP の画面でボタンが44px未満）。凍結せず直すのが望ましい。
...
1. **天真の Figma 修正** — `02 基本形` の上パディング120px・右パディング590px を 100px に。
   直ったら `--update-baseline` を実行して余白違反497件を台帳に載せる（＝それまで台帳は3件のまま）
2. **`Loop / Button` 36px** — SP のタップ領域44px未満。凍結せず直す
3. **ORDER / harness への移植** — 余白のスケール検査は LOOP にしか入っていない
4. **負債の返済** — 本物の生フレームは LP のスマホモックアップ内 `btn-primary` 3件だけ（Figma 側の作業）
5. LP の SP版を作り、`PAIR_EXEMPT_SECTIONS` から `01 LP / GOOD LOOP` を外す
6. 天真が Supabase / Sentry のプロジェクトを作成し、環境変数を登録する
7. 画面の実装に着手する。`data-loop-theme` をどこで付けるか（テナントの業態から引く）は
