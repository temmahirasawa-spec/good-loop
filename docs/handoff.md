...
> 登録した。その後30件が返済され、21件が誤検知の解消で消えて現在は3件。
> 台帳は下の「Figma の負債台帳」。**
...
| ベースライン | 空（0件）で新規作成。GOOD ORDER の51件は持ち込んでいない（同日中に54件を登録し、30件返済＋21件は誤検知の解消で現在3件。後述の台帳を参照） |
...
**現在の台帳は 3件 / 1種類。** `npm run design:figma` は緑だが、
それは「未返済の負債が3件ある」ことを意味する。
...
1. **負債の返済** — 本物の生フレームは LP のスマホモックアップ内 `btn-primary` 3件だけ（Figma 側の作業）
